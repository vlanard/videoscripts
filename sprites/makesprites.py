...
##################################
# Generate tooltip thumbnail images & corresponding WebVTT file for a video (e.g MP4). Final product is one *_sprite.png file and one *_thumbs.vtt file.
#
...
#
# NOTES on optipng: I tested using optipng in multiple times & orders, same result as using once on final sprite, ~1% reduction with default options
##################################
...
USE_SIPS = True #True to use sips if MacOSX (creates slightly smaller sprites), else set to False to use ImageMagick
USE_OPTIPNG = True # True to use optipng to further compress PNG files, False otherwise
...
def resize(files):
    ...
    #  - pass a list of files as string rather than use '*' with sips command because subprocess does not treat * as wildcard like shell does
    if USE_SIPS:
...
def makesprite(outdir,spritefile,coords,gridsize):
    ...
    # base the sprite size on the number of thumbs we need to make into a grid. TODO what about 2048x2048 limits? http://stackoverflow.com/questions/13514919/css-background-sprite-too-large-for-iphone
    ...
    doCmd(cmd)
    if USE_OPTIPNG:
        doCmd("optipng %s" % (spritefile))  #seeing about 1% size reduction (with sips or imagemagick)
...
def removespeed(videofile):
    ...
    # you won't need this if it's not relevant to your filesnames
